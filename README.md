...
**Астра** — голосовой ассистент с локальным распознаванием речи (Vosk) и синтезом (pyttsx3). Он умеет управлять системой, искать информацию в интернете, воспроизводить музыку и фильмы, а также общаться через **GigaChat**, подключаемый через **LLM Studio**, что обеспечивает локальную обработку данных.
...
> - Использует GigaChat через LLM Studio — обработка запросов локальная, но сама модель GigaChat облачная. Это позволяет получать дружелюбные и информативные ответы, на русском языке, с глубоким пониманием культуры и истории, не передавая все пользовательские команды в интернет.
...
- **Интернет**:  
  ...
  - 🔍 Поиск в DuckDuckGo  
   - 💬 Использует **GigaChat от Сбербанка** через **LLM Studio**  
